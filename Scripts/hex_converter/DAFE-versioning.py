...
if ((len(sys.argv) != 5) or (len(sys.argv[4]) != 10)):
    if (len(sys.argv) != 5):
        print "Invalid number of command line arguments.  Should be:"
        print "python DAFE-versioning.py <input.hex> <output.hex> <sensor_type> <version_string>"
    else:
    ...
    if (line_found == False):
        ...
        output_filename = sys.argv[2]+'_HW'+hw_ver+'_SW'+sw_ver+"_"+sensor_type+'.hex'
